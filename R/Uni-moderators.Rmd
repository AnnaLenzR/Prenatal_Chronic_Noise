...
db_all <- readr::read_csv(here("Data","db_lnRR.csv")) %>%
...
table(db_rr$assay_type)

assay_labels <- c(
  "Elevated Plus Maze (EPM)" = "EPM",
  "Open Field Test (OFT)" = "OFT",
  "Forced Swimming Test (FST)" = "FST",
  "Sucrose Preference Test (SPT)" = "SPT",
  "Defensive withdrawal test (DWT)" = "DWT",
  "Elevated T Maze (ETM)" = "ETM"
)


# 1) Filter each effect-size dataset (and subset the VCV accordingly)
f_rr  <- filter_low_k_single(db_rr,  VCV_rr,  MOD, min_k = 6)
...
```{r}
#| label: run_mechanism
MOD  <- "measurement_variable"
MODS <- "~ `measurement_variable`"
